...
async def next_page(bot, query):
    ...
    del_msg = f"""<b>\n\n⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>""" if settings["auto_delete"] else ''
    links = ""
    if settings["link"]:      
        btn = []
        for file_num, file in enumerate(files, start=offset+1):
            links += f"""<b>\n\n♻️ <a href=https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}>[{get_size(file.file_size)}] {file_name = file.file_name  # Keep filename exactly as stored} ({file_num})</a></b>"""
    else:
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if clicker not in [query.from_user.id, 0]:
            ...
            f"Hey {query.from_user.first_name}, This Is Not Your Query", show_alert=True
            )
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        return await query.message.edit(text=f"<b>⚡ᴛʜᴀɴᴋs ғᴏʀ ᴊᴏɪɴɪɴɢ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ😊\n\n📍Now Just Click On 'ɢᴇᴛ ʏᴏᴜʀ ғɪʟᴇ' Button Below 👇</b>",reply_markup=reply_markup)
